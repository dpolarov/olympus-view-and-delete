...
### Fixed
- **OM-1 gallery preview compatibility**: v1.3.10 requested `get_resizeimg.cgi?size=480`, but the OM-1 command list supports resize sizes 1024, 1600, 1920, and 2048. Large grid tiles now request the supported `get_resizeimg.cgi?size=1024` preview and fall back to `get_thumbnail.cgi` if it cannot be loaded.
- Grid preview cache keys now use `grid1024`, so old 160×120 thumbnails and failed/obsolete 480-preview cache entries are not reused after updating.
- **Faster full-screen previews**: the viewer now prefers `get_screennail.cgi` and falls back to `get_resizeimg.cgi?size=1920` if screennail is unavailable. On the tested OM-1 this substantially reduces preview latency while retaining high resolution.
- The compact 72×72 list view continues to use the small `get_thumbnail.cgi` endpoint to avoid unnecessary Wi-Fi traffic.

### Changed
- Version set to **1.3.11+20**.
