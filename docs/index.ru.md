...
- Крупные плитки галереи теперь используют поддерживаемое камерой **1024-px resize-превью**, а маленькая 160×120 thumbnail остаётся запасным вариантом.
- Новый cache identity `grid1024` не позволяет после обновления продолжать использовать старые низкокачественные миниатюры из кэша.
- Полноэкранный просмотр теперь сначала использует **`get_screennail.cgi`** для более быстрого высококачественного превью и при необходимости переключается на прежний **resize 1920**.
